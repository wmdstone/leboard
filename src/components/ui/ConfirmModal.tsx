...
export function ConfirmModal({
  ...
  if (isDesktop) {
    return (
      <AlertDialog open={isOpen} onOpenChange={(open) => !open && onCancel()}>
        <AlertDialogContent className="z-[1000] sm:rounded-lg shadow-soft border-0 max-w-sm p-8 text-center sm:gap-6">
          ...
          <AlertDialogFooter className="sm:justify-center flex-col sm:flex-row gap-3 sm:space-x-0 w-full mt-2">
            <AlertDialogCancel
              className="rounded-xl h-12 flex-1 m-0 bg-secondary text-secondary-foreground font-bold hover:bg-secondary/80 border-0"
              onClick={onCancel}
            >
              Cancel
            </AlertDialogCancel>
  ...
  return (
    <Drawer open={isOpen} onOpenChange={(open) => !open && onCancel()}>
      <DrawerContent className="z-[1000] rounded-t-[2rem] border-0 outline-none">
