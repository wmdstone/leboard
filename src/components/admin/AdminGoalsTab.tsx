...
export function AdminGoalsTab({ masterGoals, refreshData, categories }: any) {
  ...
  const groupedGoals = useMemo(() => {
    ...
    return Object.values(groups).filter(
      (g) => !g.category.isSystem || g.goals.length > 0,
    );
  ...
  return (
    <div className="p-4 sm:p-8">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-8">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-3">
            Kelola kategori dan template Tugasnya.
          </p>
        </div>
                        ...
                        options={[
                          {
                            label: "Edit",
                            onClick: () => {
                              setEditCatData(group.category);
                              setEditCatName(group.category.name);
                            },
                            icon: (
                      ...
                      {group.goals.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                          {group.goals.map((mg: any, mgIndex: number) => (
                            <Card
                              key={`${mg.id}-${mgIndex}`}
                              className="rounded-xl border border-border shadow-none hover:shadow-soft transition-shadow group relative flex flex-col justify-between"
                            >
                              <CardContent className="p-4 space-y-2">
                                <div className="flex justify-between items-start gap-2">
                                  <h4
                                    className="font-bold text-foreground leading-tight flex-1 pt-1"
                                    title={mg.title}
                                  >
                                    {mg.title}
                                  </h4>
                                ...
                                <p
                                  className="text-xs text-muted-foreground italic leading-relaxed line-clamp-2 pr-4"
      ...
      <ConfirmModal
        ...
        message={`Apakah Anda yakin ingin menghapus ${deleteGoalConfirm?.title}? Santri akan tetap menyimpan referensi ini tetapi datanya tidak akan disinkronkan.`}
      ...
      <ConfirmModal
        ...
        message={`Are you sure you want to delete ${deleteCatConfirm?.name}? Tugas yang menggunakan Kategori ini akan dipindahkan ke Tidak Diketahui.`}
      ...
      <Card className="w-full max-w-md rounded-xl shadow-2xl border-border bg-card overflow-hidden">
        ...
        <div className="p-6 border-t border-border bg-secondary/20 flex justify-end gap-3">
          ...
          <Button
            ...
            className="rounded-xl h-12 font-bold shadow-primary-glow"
          >
            Simpan Model
          </Button>
        </div>
